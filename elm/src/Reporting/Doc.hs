...
fromText txt = P.text (Text.unpack txt)

fromName :: N.Name -> P.Doc
fromName name = P.text (N.toString name)

fromInt :: Int -> P.Doc
fromInt n = P.text (show n)

-- TO STRING
toAnsi :: Handle -> P.Doc -> IO ()
toAnsi handle doc = P.displayIO handle (P.renderPretty 1 80 doc)

toString :: P.Doc -> String
toString doc = P.displayS (P.renderPretty 1 80 (P.plain doc)) ""

toLine :: P.Doc -> String
toLine doc = P.displayS (P.renderPretty 1 (div maxBound 2) (P.plain doc)) ""

-- FORMATTING
stack :: [P.Doc] -> P.Doc
stack docs = P.vcat (List.intersperse "" docs)

reflow :: String -> P.Doc
reflow paragraph = P.fillSep (map P.text (words paragraph))

commaSep :: P.Doc -> (P.Doc -> P.Doc) -> [P.Doc] -> [P.Doc]
commaSep conjunction addStyle names =
  case names of
    [name] -> [addStyle name]
    [name1, name2] -> [addStyle name1, conjunction, addStyle name2]
    _ ->
      map (\name -> addStyle name <> ",") (init names) ++
      [conjunction, addStyle (last names)]
...
toSimpleHint message = toFancyHint (map P.text (words message))

toFancyHint :: [P.Doc] -> P.Doc
toFancyHint chunks = P.fillSep (P.underline "Hint" <> ":" : chunks)
...
link word before fileName after =
  ...
  P.text (makeLink fileName) : map P.text (words after)
...
-- TODO: Make version not static
makeLink :: String -> String
makeLink fileName =
  "<https://elm-lang.org/" <> Elm.Version.version <> "/" <> fileName <> ">"
...
ordinal index = intToOrdinal (Index.toHuman index)

intToOrdinal :: Int -> String
intToOrdinal number =
  let remainder10 = number `mod` 10
      remainder100 = number `mod` 100
      ending
...
encode doc = E.array (toJsonHelp noStyle [] (P.renderPretty 1 80 doc))
...
noStyle :: Style
noStyle = Style False False Nothing
...
toJsonHelp style revChunks simpleDoc =
  case simpleDoc of
    ...
    P.SEmpty -> [encodeChunks style revChunks]
    P.SChar char rest -> toJsonHelp style (TB.singleton char : revChunks) rest
...
spaces n = TB.fromText (Text.replicate n " ")
...
encodeChunks (Style bold underline color) revChunks =
  ...
  in case color of
       Nothing
         | not bold && not underline -> E.text text
       _ ->
         E.object
           [ ("bold", E.bool bold)
